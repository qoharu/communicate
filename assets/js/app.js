    ...
    var latestMessage, latestSender;
    ...
    function loadCallOptions() {
        if (window.localStorage) {
            ...
            callstatus.innerHTML = '<i>Video ' + (bDisableVideo ? 'disabled' : 'enabled') + '</i>';
        }
    }
    ...
    function sipRegister() {
        ...
        try {
            ...
            if (!txtRealm.value || !txtPrivateIdentity.value || !txtPublicIdentity.value) {
                ...
                registerstatus.innerHTML = '<b>Please fill madatory fields (*)</b>';
                btnRegister.disabled = false;
                return;
            }
            ...
            if (!o_impu || !o_impu.s_user_name || !o_impu.s_host) {
                ...
                registerstatus.innerHTML = "<b>[" + txtPublicIdentity.value + "] is not a valid Public identity</b>";
                btnRegister.disabled = false;
                return;
            }
            ...
            if (oSipStack.start() != 0) {
                ...
                registerstatus.innerHTML = '<b>Failed to start the SIP stack</b>';
            }
            else return;
        }
        catch (e) {
            
            registerstatus.innerHTML = "<b>2:" + e + "</b>";
        }
        btnRegister.disabled = false;
    }
    ...
    function sipCall(s_type) {
        ...
        if (oSipStack && !oSipSessionCall && !tsk_string_is_null_or_empty(txtPhoneNumber.value)) {
            ...
            if (oSipSessionCall.call(txtPhoneNumber.value) != 0) {
                ...
                callstatus.innerHTML = 'Failed to make call';
            ...
            callstatus.innerHTML = '<i>Connecting...</i>';
            oSipSessionCall.accept(oConfigCall);
        }
    }

    function sipMessage(dest,msg){
        var sesianyar = oSipStack.newSession('message');
        sesianyar.send(dest,msg,'text/plain;charset=utf8');
    }
    ...
    function sipShareScreen() {
        if (SIPml.getWebRtcType() === 'w4a') {
            ...
            if (!oSipSessionCall) {
                callstatus.innerHTML = '<i>No active session</i>';
                return;
            }
            if (oSipSessionCall.bfcpSharing) {
                if (oSipSessionCall.stopBfcpShare(oConfigCall) != 0) {
                    callstatus.innerHTML = 'Failed to stop BFCP share';
                }
                else {
                ...
                if (oSipSessionCall.startBfcpShare(oConfigCall) != 0) {
                    callstatus.innerHTML = 'Failed to start BFCP share';
                }
                else {
        ...
        if (oSipSessionCall) {
            ...
            if (!tsk_string_is_null_or_empty(s_destination)) {
                ...
                if (oSipSessionCall.transfer(s_destination) != 0) {
                    callstatus.innerHTML = '<i>Call transfer failed</i>';
                    btnTransfer.disabled = false;
                    return;
                }
                callstatus.innerHTML = '<i>Transfering the call...</i>';
            }
        }
    }
    ...
    function sipToggleHoldResume() {
        if (oSipSessionCall) {
            ...
            callstatus.innerHTML = oSipSessionCall.bHeld ? '<i>Resuming the call...</i>' : '<i>Holding the call...</i>';
            i_ret = oSipSessionCall.bHeld ? oSipSessionCall.resume() : oSipSessionCall.hold();
            if (i_ret != 0) {
                callstatus.innerHTML = '<i>Hold / Resume failed</i>';
    ...
    function sipToggleMute() {
        if (oSipSessionCall) {
            ...
            callstatus.innerHTML = bMute ? '<i>Mute the call...</i>' : '<i>Unmute the call...</i>';
            i_ret = oSipSessionCall.mute('audio'/*could be 'video'*/, bMute);
            if (i_ret != 0) {
                callstatus.innerHTML = '<i>Mute / Unmute failed</i>';
                return;
            }
    ...
    function sipHangUp() {
        if (oSipSessionCall) {
            callstatus.innerHTML = '<i>Terminating the call...</i>';
    ...
    function uiCallTerminated(s_description){
        ...
        callstatus.innerHTML = "<i>" + s_description + "</i>";
        ...
        setTimeout(function () { if (!oSipSessionCall) callstatus.innerHTML = ''; }, 2500);
    }
    ...
    function onSipEventStack(e /*SIPml.Stack.Event*/) {
        ...
        switch (e.type) {
            case 'started':
                {
                    ...
                    try {
                        ...
                        registerstatus.value = registerstatus.innerHTML = "<b>1:" + e + "</b>";
                        btnRegister.disabled = false;
                    }
                    break;
                }
            case 'stopping': case 'stopped': case 'failed_to_start': case 'failed_to_stop':
                {
                    ...
                    localStorage.removeItem("cmn-islogin");
                    window.location="#/";

                    callstatus.innerHTML = '';
                    
                    registerstatus.innerHTML = bFailure ? "<i>Disconnected: <b>" + e.description + "</b></i>" : "<i>Disconnected</i>";
                    break;
                }
            ...
            case 'i_new_call':
                {
                    if (oSipSessionCall) {
                        ...
                        callstatus.innerHTML = "<i>Incoming call from [<b>" + sRemoteNumber + "</b>]</i>";
                        showNotifICall(sRemoteNumber);
                        $("#caller").html(sRemoteNumber);
                        $('.notif-mantap').slideDown();
            ...
            case 'i_new_message':
                {
                    e.newSession.accept();
                    nama = e.newSession.getRemoteUri();
                    nama = nama.replace('<sip:','');
                    nama = nama.replace('>','');
                    pesan = e.getContentString();

                    if (latestMessage != pesan) {
                        newchat(nama,localStorage.getItem("cmn-user"),pesan);
                    };
    ...
    function onSipEventSession(e /* SIPml.Session.Event */) {
        ...
        switch (e.type) {
            case 'connecting': case 'connected':
                {   
                    localStorage.setItem("cmn-islogin", true);
                    window.location = "#/home";
                    var bConnected = (e.type == 'connected');
                    if (e.session == oSipSessionRegister) {
                        uiOnConnectionEvent(bConnected, !bConnected);
                        
                        registerstatus.innerHTML = "<i>" + e.description + "</i>";
                    }
                        ...
                        callstatus.innerHTML = "<i>" + e.description + "</i>";
            ...
            case 'i_ao_request':
                {
                    if(e.session == oSipSessionCall){
                        ...
                        if (iSipResponseCode == 180 || iSipResponseCode == 183) {
                            startRingbackTone();
                            callstatus.innerHTML = '<i>Remote ringing...</i>';
                        }
                    }
                    break;
                }
            ...
            case 'm_early_media':
                {
                    if(e.session == oSipSessionCall){
                        stopRingbackTone();
                        stopRingTone();
                        callstatus.innerHTML = '<i>Early media started</i>';
                    }
                    break;
                }
            ...
            case 'm_local_hold_ok':
                {
                    if(e.session == oSipSessionCall){
                        ...
                        btnHoldResume.disabled = false;
                        callstatus.innerHTML = '<i>Call placed on hold</i>';
            ...
            case 'm_local_hold_nok':
                {
                    if(e.session == oSipSessionCall){
                        ...
                        btnHoldResume.value = 'Hold';
                        btnHoldResume.disabled = false;
                        callstatus.innerHTML = '<i>Failed to place remote party on hold</i>';
                    }
                    break;
                }
            case 'm_local_resume_ok':
                {
                    if(e.session == oSipSessionCall){
                        ...
                        btnHoldResume.value = 'Hold';
                        btnHoldResume.disabled = false;
                        callstatus.innerHTML = '<i>Call taken off hold</i>';
            ...
            case 'm_local_resume_nok':
                {
                    if(e.session == oSipSessionCall){
                        oSipSessionCall.bTransfering = false;
                        btnHoldResume.disabled = false;
                        callstatus.innerHTML = '<i>Failed to unhold call</i>';
                    }
                    break;
                }
            case 'm_remote_hold':
                {
                    if(e.session == oSipSessionCall){
                        callstatus.innerHTML = '<i>Placed on hold by remote party</i>';
                    }
                    break;
                }
            case 'm_remote_resume':
                {
                    if(e.session == oSipSessionCall){
                        callstatus.innerHTML = '<i>Taken off hold by remote party</i>';
                    }
                    break;
                }
            case 'm_bfcp_info':
                {
                    if(e.session == oSipSessionCall){
                        callstatus.innerHTML = 'BFCP Info: <i>'+ e.description +'</i>';
                    }
                    break;
                }
            ...
            case 'o_ect_trying':
                {
                    if(e.session == oSipSessionCall){
                        callstatus.innerHTML = '<i>Call transfer in progress...</i>';
                    }
                    break;
                }
            case 'o_ect_accepted':
                {
                    if(e.session == oSipSessionCall){
                        callstatus.innerHTML = '<i>Call transfer accepted</i>';
                    }
                    break;
                }
            ...
            case 'i_ect_completed':
                {
                    if(e.session == oSipSessionCall){
                        callstatus.innerHTML = '<i>Call transfer completed</i>';
                        btnTransfer.disabled = false;
            ...
            case 'i_ect_notify':
                {
                    if(e.session == oSipSessionCall){
                        callstatus.innerHTML = "<i>Call Transfer: <b>" + e.getSipResponseCode() + " " + e.description + "</b></i>";
